...
const ProjectDropdown = ({ id, navigate }) => {
    ...
    const handleDelete = async () => {
        try {
            const response = await axios.delete(`${process.env.REACT_APP_BASE_URL}/projects/${id}`);
        ...
        <>
            <Popover className={`relative`}>
                <Popover.Button className="duration-75 rounded-sm focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" width={16} className="text-slate-500/90" viewBox="0 0 512 512">
